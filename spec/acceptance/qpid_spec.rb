...
describe 'certs' do
  ...
  context 'with default params' do
    ...
    describe nssdb_certificate(nssdb_dir, name: 'ca') do
      ...
      its(:subject) { should match(/CN=#{fqdn},OU=SomeOrgUnit,O=Katello,L=Raleigh,ST=North Carolina,C=US/) }
      its(:issuer) { should match(/CN=#{fqdn},OU=SomeOrgUnit,O=Katello,L=Raleigh,ST=North Carolina,C=US/) }
    end

    describe nssdb_certificate(nssdb_dir, name: 'broker') do
      its(:exit_status) { should eq 0 }
      its(:subject) { should match(/CN=#{fqdn},OU=SomeOrgUnit,O=pulp,ST=North Carolina,C=US/) }
      its(:issuer) { should match(/CN=#{fqdn},OU=SomeOrgUnit,O=Katello,L=Raleigh,ST=North Carolina,C=US/) }
    end
